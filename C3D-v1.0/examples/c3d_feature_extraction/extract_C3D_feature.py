...
def extract_frames(video, start_frame,end_frame,frame_dir, num_frames_to_extract=16):
    ...
    for frame_count in range(num_frames_to_extract):
        # dist=max(int((end_frame-start_frame)/num_frames_to_extract),1)
        dist=1
        frame_num = frame_count*dist + start_frame
        if frame_count==0:
...
def c3d_extractor(video_file='/home/kraus/PycharmProjects/Real-World-Anomaly-Detection/SampleVideos/Explosion025_x264.mp4',\
                  outdir='/home/kraus/PycharmProjects/Real-World-Anomaly-Detection/SampleVideos'):
...
#         sys.exit(-1)
    ...
    tmp_dir = '/home/kraus/PycharmProjects/Real-World-Anomaly-Detection/tmp'
    ...
    gap=int(num_frames/32)
    feature_all = []
    # for segment in range(0,num_frames-gap,gap):
    #     if gap < int(sample_every_N_sec * fps/32):
    #         start_frame = ((gap - num_frames_per_clip) / 2)+segment
    #         start_frames = [start_frame]
    #     else:
    #         frame_inc = int((sample_every_N_sec * fps)/32)
    #         start_frame = frame_inc / 2+segment
    #         # make sure not to reach the edge of the video
    #         end_frame = (min(gap, int(max_processing_sec * fps)) - \
    #                     num_frames_per_clip)+segment
    #         start_frames = []
    ...
    start_frame=0
    for segment in range(0, 32):
        start_frame=int(gap*segment)
        end_frame=int(start_frame+gap-1)
        ...
        #extract_frames
        extract_frames(video_file,start_frame,end_frame, frame_dir)
